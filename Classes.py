...
class Patient:
    def __init__(self, id, UTI_prob):
        """ initiates a patient
        :param id: ID of the patient
        :param UTI_prob: probability of UTI during a time-step (must be in [0,1])
        """
        self._id = id
        ...
        self._UTIProb = UTI_prob
        self._healthState = HealthStat.NO_UTI  # assuming all patients are alive at the beginning
        self._numberUTIs = 0
    ...
    def simulate(self, n_time_steps):
        ...
        # while the patient has no UTI and simulation length is not yet reached
        while self._healthState == HealthStat.NO_UTI and t < n_time_steps:
            # determine if the patient will get a UTI during this time-step
            if self._rnd.sample() < self._UTIProb:
                self._healthState = HealthStat.UTI
                self._numberUTIs = t + 1  # assuming UTIs occur at the end of this period
    ...
    def get_number_UTIs(self):
        ...
        if self._healthState == HealthStat.NO_UTI:
...
class Cohort:
    def __init__(self, id, pop_size, UTI_prob):
        """ create a cohort of patients
        :param id: cohort ID
        :param pop_size: population size of this cohort
        :param mortality_prob: probability of UTI for each patient in this cohort over a time-step (must be in [0,1])
        """
        self._initialPopSize = pop_size # initial population size
        self._patients = []      # list of patients
        self._numberUTIs = []    # list to store survival time of each patient

        # populate the cohort
        for i in range(pop_size):
            # create a new patient (use id * pop_size + n as patient id)
            patient = Patient(id * pop_size + i, UTI_prob)
    ...
    def simulate(self, n_time_steps):
        ...
        for patient in self._patients:
            ...
            value = patient.get_number_UTIs()
            if not (value is None):
                self._numberUTIs.append(value)
